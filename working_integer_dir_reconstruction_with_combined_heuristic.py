    ...
    while len(h) > 0:
        ...
        if current_state == goal_state_string:
            ...
            while current_state != initial_state_string:
                ...
                if last_move[current_state] == 0:
                    minPath.append('Down')
                    cur_zero_column += 1
                elif last_move[current_state] == 1:
                    minPath.append('Left')
                    cur_zero_row += 1
                elif last_move[current_state] == 2:
                    minPath.append('Up')
                    cur_zero_column -= 1
                elif last_move[current_state] == 3:
        ...
        for move in range(4):
            ...
            if 0 <= next_zero_row and next_zero_row < 4 and 0 <= next_zero_column and next_zero_column < 4:
                ...
                if next_state not in minimum_cost_list or (next_state in minimum_cost_list and minimum_cost_list.get(next_state) > current_path_cost+1+heuristic(next_state)):
                    ...
                    heappush(h, [[minimum_cost_list[next_state], current_path_cost+1], next_state])

...
def FindMinimumPath(initialState,goalState):
    ...
    for i in range(16):
        tile = 0
        if initial_state_string[i].isdigit():
            tile = int(initial_state_string[i])
            if tile == 0:
                continue
        else:
            tile = ord(initial_state_string[i])-ord('A')+10
        cur_row = int(i/4)
        cur_column = i%4
        goal_row = int(tile/4)
        goal_column = tile%4
    ...
    heuristic = manhatten_distance

    minimum_cost_list = dict()
    minimum_cost_list[initial_state_string] = heuristic
    last_move = dict()
    ...
    heap = []
    heappush(heap, [[0+heuristic, 0], initial_state_string])
    ...
    while len(heap) > 0:
        ...
        if current_state == goal_state_string:
            ...
            while current_state != initial_state_string:
                ...
                # GETTING THE INDEX OF 0 IN THE STATE STRING
                cur_zero_position = -1
                for position in range(16):
                    if current_state[position] == '0':
                        cur_zero_position = position
        ...
        for move in range(4):
            ...
            if 0 <= next_zero_row and next_zero_row < 4 and 0 <= next_zero_column and next_zero_column < 4:
                ...
                for i in range(16):
                    tile = 0
                    if next_state[i].isdigit():
                        tile = int(next_state[i])
                        if tile == 0:
                            continue
                    else:
                        tile = ord(next_state[i])-ord('A')+10
                    cur_row = int(i/4)
                    cur_column = i%4
                    goal_row = int(tile/4)
                    goal_column = tile%4
                ...
                if next_state not in minimum_cost_list or (next_state in minimum_cost_list and minimum_cost_list.get(next_state) > current_path_cost+1+heuristic):
                    nodesGenerated += 1
                    minimum_cost_list[next_state] = current_path_cost+1+heuristic
                    if move == 0:
